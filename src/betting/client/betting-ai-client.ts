...
function logStageMetrics(
  ...
  logger.info(
    `  ✓ combined ${payload.home} vs ${payload.away}: ${latencyMs}ms, model=${model}, requests=${requestCount}, input=${response.usage.promptTokens}, output=${response.usage.completionTokens}, fallback=${usedFallback ? "yes" : "no"}`,
  );
}
...
export function buildCombinedSystemPrompt(): string {
  return [
    ...
    "   - Chỉ đề xuất kèo có odds > 1.8 (không lấy kèo odds thấp dù tỷ lệ thắng cao, ưu tiên kèo vừa có xác suất thắng tốt vừa có odds > 1.8).",
    ...
    "   - Nếu không có kèo nào đủ tin cậy (edge không rõ ràng) thì trả picks = [] (mảng rỗng = đứng ngoài).",
    "3. Dự đoán tỉ số chính xác (predictedScore) kèm % tự tin (0-100), dựa trên odds correct score và các market khác.",
    "4. (tùy chọn) 1 câu note ngắn nếu có điểm đáng chú ý khác.",
    "",
...
function normalizeCombinedMatch(
  ...
  const note = toText(match.note);
  if (note && isStandAsideRecommendation(note)) {
    return {
...
function parseCombinedAnalysisResponse(
  ...
  try {
    ...
    const normalizedMatches = parsed.matches.map((match) =>
    ...
    logger.warn(
      `  ! Combined analysis parse error: ${error instanceof Error ? error.message : String(error)}`,
    );
    return null;
  }
}
