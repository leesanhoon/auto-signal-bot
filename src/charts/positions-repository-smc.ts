...
import { getConfiguredPendingOrderExpiryRuns } from "./smc-config-env.js";
import {
  buildOpenPositionInsertRow,
  deriveManagementPatch,
  type OpenPositionManagementPatch,
  type PositionDecisionOutcome,
} from "./position-engine-smc.js";
import { buildClosedPositionSnapshot, type ClosedPositionRecord } from "./performance-tracking-smc.js";
...
export async function saveOpenPosition(setup: TradeSetup): Promise<boolean> {
  ...
  const { data: existing, error: existingError } = await (getDb().from("open_positions_smc") as any)
    .select("id")
    .eq("status", "open")
    .eq("pair", setup.pair)
    .limit(1);
  ...
  const { error } = await (getDb().from("open_positions_smc") as any).insert(row);
...
export async function savePendingOrder(setup: TradeSetup): Promise<boolean> {
  ...
  const { data: existing, error: existingError } = await (getDb().from("pending_orders_smc") as any)
    .select("id")
    .eq("status", "PENDING")
    .eq("pair", setup.pair)
    .limit(1);
  ...
  const { error } = await (getDb().from("pending_orders_smc") as any).insert(row);
...
export async function loadPendingOrders(): Promise<PendingOrder[]> {
  const { data, error } = await (getDb().from("pending_orders_smc") as any)
    .select(
...
export async function updatePendingOrder(id: number, patch: PendingOrderUpdate): Promise<void> {
  const { error } = await (getDb().from("pending_orders_smc") as any)
    .update({
...
export async function loadClosedPositions(since?: string): Promise<ClosedPositionRecord[]> {
  let query = (getDb().from("open_positions_smc") as any)
    .select(
