...
describe("charts/binance-execution-smc reconcileBinancePosition", () => {
  ...
  describe("Finding phụ #2: manual close detection (Task 01)", () => {
    test("no SL/TP filled + getPositionAmount = 0 -> manual close phat hien, ket luan CLOSE", async () => {
      const position = { ...basePosition, binanceSlOrderId: 111, binanceTp1OrderId: 222, binanceTp2OrderId: 333 };
      // SL, TP1, TP2 khong FILLED (all NEW)
      clientState.getOrderStatus
        .mockResolvedValueOnce({ status: "NEW" }) // SL not filled
        .mockResolvedValueOnce({ status: "NEW" }) // TP2 not filled
        .mockResolvedValueOnce({ status: "NEW" }); // TP1 not filled
      // getPositionAmount = 0: user da dong tay tren Binance app
      clientState.getPositionAmount.mockResolvedValueOnce(0);
      // best-effort: huy cac order con lai
      clientState.cancelOrder
        .mockResolvedValueOnce({ orderId: 111 }) // Cancel SL
        .mockResolvedValueOnce({ orderId: 222 }) // Cancel TP1
        .mockResolvedValueOnce({ orderId: 333 }); // Cancel TP2

      const result = await reconcileBinancePosition(position);
      ...
      expect(clientState.cancelOrder).toHaveBeenCalledWith("BTCUSDT", 111); // SL
      expect(clientState.cancelOrder).toHaveBeenCalledWith("BTCUSDT", 222); // TP1
      expect(clientState.cancelOrder).toHaveBeenCalledWith("BTCUSDT", 333); // TP2
    });
    ...
    test("no SL/TP filled + getPositionAmount = 0, cancel orders fail -> best-effort, still return CLOSE", async () => {
      ...
      clientState.cancelOrder
        .mockResolvedValueOnce(new Error("cancel SL failed"))
        .mockResolvedValueOnce(new Error("cancel TP1 failed"))
        .mockResolvedValueOnce(new Error("cancel TP2 failed"));
    ...
    test("no SL/TP filled + getPositionAmount return Error -> fallback HOLD, khong conclude CLOSE", async () => {
      const position = { ...basePosition, binanceSlOrderId: 111, binanceTp1OrderId: 222, binanceTp2OrderId: null };
      clientState.getOrderStatus
        .mockResolvedValueOnce({ status: "NEW" }) // SL not filled
        .mockResolvedValueOnce({ status: "NEW" }); // TP1 not filled
    ...
    test("no SL/TP filled + getPositionAmount = 0 nhung KHONG co order id -> entry LIMIT/STOP dang cho khop, ket luan HOLD (khong phai dong thu cong)", async () => {
      ...
      clientState.getOrderStatus
        .mockResolvedValueOnce({ status: "NEW" }) // SL not filled (but doesn't exist)
        .mockResolvedValueOnce({ status: "NEW" }); // TP2 not filled (but doesn't exist)
      clientState.getOrderStatus
        .mockResolvedValueOnce({ status: "NEW" }); // TP1 not filled (but doesn't exist)
