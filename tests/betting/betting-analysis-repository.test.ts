...
describe("betting/betting-analysis-repository", () => {
  ...
  describe("savePlanCache", () => {
    test("upserts plan cache with date as key", async () => {
      const mockPlan = {
        summary: "Test plan",
        matches: [],
        parlays: [],
      ...
      const before = Date.now();
      await bettingRepo.savePlanCache("2026-07-03", ["game-1", "game-2"], mockPlan);
      const after = Date.now();

      expect(repoState.from).toHaveBeenCalledWith("betting_plan_cache");
      const upsertPayload = repoState.from().upsert.mock.calls[0][0];
      expect(upsertPayload.date).toBe("2026-07-03");
      expect(upsertPayload.game_ids).toEqual(["game-1", "game-2"]);
      expect(upsertPayload.plan).toEqual(mockPlan);
      expect(upsertPayload.created_at).toBeDefined();
      const createdAtMs = new Date(upsertPayload.created_at).getTime();
      expect(createdAtMs).toBeGreaterThanOrEqual(before - 1);
      expect(createdAtMs).toBeLessThanOrEqual(after + 1);
    ...
    test("throws on DB error", async () => {
      ...
      await expect(
        bettingRepo.savePlanCache("2026-07-03", ["game-1"], {
          summary: "Test",
          matches: [],
          parlays: [],
          remainingSingles: [],
        }),
      ).rejects.toThrow("savePlanCache failed");
    });
  });

  describe("loadRecentPlanCache", () => {
    test("returns null when gameIds empty", async () => {
      const result = await bettingRepo.loadRecentPlanCache("2026-07-03", [], 30_000);
      expect(result).toBeNull();
      expect(repoState.from).not.toHaveBeenCalled();
    });

    test("returns plan when found and gameIds match", async () => {
      const mockPlan = {
        summary: "Test plan",
        matches: [],
        parlays: [],
      ...
      const chain: Record<string, ReturnType<typeof vi.fn>> = {
        ...
        eq: vi.fn(() => chain),
        gte: vi.fn(async () => ({
          data: [{ plan: mockPlan, game_ids: ["game-1", "game-2"], created_at: "2026-07-03T10:00:00Z" }],
          error: null,
        })),
      };
      ...
      const result = await bettingRepo.loadRecentPlanCache("2026-07-03", ["game-1", "game-2"], 30_000);

      expect(result).toEqual(mockPlan);
    });

    test("returns null when gameIds don't match (different count)", async () => {
      const mockPlan = {
        summary: "Test plan",
        matches: [],
        parlays: [],
      ...
      repoState.selectResult = {
        data: [{ plan: mockPlan, game_ids: ["game-1"], created_at: "2026-07-03T10:00:00Z" }],
        error: null,
      };
    ...
    test("returns null when gameIds don't match (different content)", async () => {
      const mockPlan = {
        summary: "Test plan",
        matches: [],
        parlays: [],
      ...
      repoState.selectResult = {
        data: [{ plan: mockPlan, game_ids: ["game-1", "game-3"], created_at: "2026-07-03T10:00:00Z" }],
        error: null,
      };
    ...
    test("returns null when DB error", async () => {
      repoState.selectResult = { data: null, error: { message: "DB error" } };

      const result = await bettingRepo.loadRecentPlanCache("2026-07-03", ["game-1"], 30_000);
